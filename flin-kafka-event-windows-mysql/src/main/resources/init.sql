...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `savedataeventhistory`(in id varchar(32),in eventDate date,in hn varchar(3),in lastValue int )
BEGIN
	declare SQL_FOR_UPDATE varchar(500);
	SET SQL_FOR_UPDATE = CONCAT("insert into `dataeventhistory`(`id`, `eventDate`,`",hn,"`) values(?, ?, ?) ON duplicate key update`",hn,"`=VALUES(`",hn,"`)");
